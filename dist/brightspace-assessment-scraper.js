...
function BrightspaceApi2(brightspaceBase, brightspaceApiBase) {
    const brightspaceApi = {
        assessmentList: (organizationId2) =>
            new URL(`/d2l/api/le/1.8/${organizationId2}/dropbox/folders/`, brightspaceApiBase).toString(),
        classList: (organizationId2) =>
            new URL(`/d2l/api/le/1.8/${organizationId2}/classlist/`, brightspaceBase).toString(),
        sectionList: (organizationId2) =>
            new URL(`/d2l/api/lp/1.8/${organizationId2}/sections/`, brightspaceApiBase).toString(),
        authToken: () => new URL(`/d2l/lp/auth/oauth2/token`, brightspaceBase).toString(),
        referrer: (organizationId2) =>
            new URL('/d2l/ap/insights/classEngagement/View?ou=' + organizationId2, brightspaceBase).toString(),
        rubricsCriteria: (organizationId2, rubricId) =>
            new URL(`/organizations/${organizationId2}/${rubricId}/groups`, brightspaceApiBase).toString(),
        rubricsScore: () => new URL('/d2l/lms/competencies/rubric/rubrics_assessment_results.d2l', brightspaceBase),
        gradesExport: (organizationId2) =>
            new URL(
    ...
    async function getRubricCriteria(organizationId2, rubricId, jwt) {
        ...
        return Promise.all(
            links.map((url) =>
                fetch(url, { headers: { authorization: `Bearer ${jwt}` } }).then((response2) => response2.json()),
            ),
        );
    }
    async function getStudentRubricScore(orgId, evalObjectId, rubricId, studentId, jwt) {
        ...
        const studentRubricCriteriaAssessmentLinks = studentRubricCriteriaData.entities
            .filter((entity) => entity.class[0] === 'criterion-assessment-links')
            .map((entity) => entity.links[2].href);
    ...
    function getAssessmentList(organizationId2) {
        return getAccessToken2(organizationId2)
            .then((jwt) =>
                fetch(brightspaceApi.assessmentList(organizationId2), {
                    headers: { authorization: `Bearer ${jwt}` },
                }),
            )
...
function generateSasCsvData(studentResult, title) {
    const csv = [
        [
            defaults_default.SAS_CSV_CLASS_COLUMN_NAME,
            'Name',
            'Student Id',
            title,
            defaults_default.SAS_CSV_GRADES_COLUMN_NAME,
        ],
    ];
    csv.push(
        ...studentResult
...
function Scraper(brightspaceApi) {
    async function getStudentList(orgId, jwt) {
        ...
        const students = classList.filter(
            ({ RoleId: roleId }) =>
                roleId === defaults_default.ROLES.STUDENT || roleId === defaults_default.ROLES.LEARNER,
        );
...
export { BrightspaceRubricScraper as default };
